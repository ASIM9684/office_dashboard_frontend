...
export default function DepartmentPage() {
  ...
  useEffect(() => {
    const fetchDepartment = async () => {
      try {
        const response = await axios.get("http://localhost:8080/getDepartments");
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this Deaprtment?")) {
      axios
        .delete(`http://localhost:8080/deleteDepartment/${id}`)
        .then(() => {
     ...
     <div className="max-w-full mx-auto">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-4">
        <div className="flex items-center gap-2">
          <UserCog className="text-slateblue w-6 h-6" />
